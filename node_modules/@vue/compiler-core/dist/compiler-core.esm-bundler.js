...
const errorMessages = {
    ...
    [49 /* X_INVALID_EXPRESSION */]: `Error parsing JavaScript expression: `,
...
const PORTAL = Symbol((process.env.NODE_ENV !== 'production') ? `Portal` : ``);
...
function getInnerRange(loc, offset, length) {
    (process.env.NODE_ENV === 'test') && assert(offset <= loc.source.length);
    ...
    if (length != null) {
        (process.env.NODE_ENV === 'test') && assert(offset + length <= loc.source.length);
...
function parseChildren(context, mode, ancestors) {
    ...
    while (!isEnd(context, mode, ancestors)) {
        (process.env.NODE_ENV === 'test') && assert(context.source.length > 0);
...
function parseCDATA(context, ancestors) {
    ...
    (process.env.NODE_ENV === 'test') && assert(startsWith(context.source, '<![CDATA['));
    advanceBy(context, 9);
    ...
    if (context.source.length === 0) {
        ...
        (process.env.NODE_ENV === 'test') && assert(startsWith(context.source, ']]>'));
        advanceBy(context, 3);
    }
    return nodes;
}
...
function parseBogusComment(context) {
    (process.env.NODE_ENV === 'test') && assert(/^<(?:[\!\?]|\/[^a-z>])/i.test(context.source));
    const start = getCursor(context);
...
function parseAttribute(context, nameSet) {
    (process.env.NODE_ENV === 'test') && assert(/^[^\t\r\n\f />]/.test(context.source));
    // Name.
    const start = getCursor(context);
...
function parseInterpolation(context, mode) {
    ...
    (process.env.NODE_ENV === 'test') && assert(startsWith(context.source, open));
...
function parseText(context, mode) {
    ...
    (process.env.NODE_ENV === 'test') && assert(endIndex > 0);
    const start = getCursor(context);
...
function advanceBy(context, numberOfCharacters) {
    ...
    (process.env.NODE_ENV === 'test') && assert(numberOfCharacters <= source.length);
...
const transformRef = node => {
    ...
    if (!ref)
        return;
    const refKey = ref.type === 6 /* ATTRIBUTE */
        ? ref.value
...
asRawStatements = false) {
    ...
    try {
        ...
        context.onError(createCompilerError(49 /* X_INVALID_EXPRESSION */, node.loc, undefined, e.message));
        return node;
    }
    const ids = [];
    ...
    for (let i = 0; i < children.length; i++) {
        ...
        if ((vIf = findDir(slotElement, 'if'))) {
            ...
            if (prev && isTemplateNode(prev) && findDir(prev, 'if')) {
                ...
                children.splice(i, 1);
                i--;
                (process.env.NODE_ENV === 'test') && assert(dynamicSlots.length > 0);
