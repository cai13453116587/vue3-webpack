...
function callWithAsyncErrorHandling(fn, instance, type, args) {
    if (shared.isFunction(fn)) {
        ...
        if (res != null && !res._isVue && shared.isPromise(res)) {
            res.catch((err) => {
                handleError(err, instance, type);
            });
        }
...
function renderComponentRoot(instance) {
    ...
    try {
        ...
        if (Component.inheritAttrs !== false &&
            ...
            if (result.shapeFlag & 1 /* ELEMENT */ ||
                ...
                if (result.dynamicChildren !== null) {
                    result.patchFlag |= 16 /* FULL_PROPS */;
                }
            }
        ...
        // inherit vnode hooks
        if (vnodeHooks !== shared.EMPTY_OBJ) {
...
const PortalImpl = {
    __isPortal: true,
    process(n1, n2, container, anchor, parentComponent, parentSuspense, isSVG, optimized, { mc: mountChildren, pc: patchChildren, pbc: patchBlockChildren, m: move, o: { insert, querySelector, setElementText, createComment } }) {
        const targetSelector = n2.props && n2.props.target;
        const { patchFlag, shapeFlag, children } = n2;
        if (n1 == null) {
            // insert an empty node as the placeholder for the portal
            insert((n2.el = createComment(`portal`)), container, anchor);
            if ( shared.isString(targetSelector) && !querySelector) {
                warn(`Current renderer does not support string target for Portals. ` +
                    `(missing querySelector renderer option)`);
            }
            const target = (n2.target = shared.isString(targetSelector)
            ...
            if (targetSelector !== (n1.props && n1.props.target)) {
                const nextTarget = (n2.target = shared.isString(targetSelector)
                    ? querySelector(targetSelector)
                    : targetSelector);
                if (nextTarget != null) {
                    // move content
...
function createVNode(type, props = null, children = null, patchFlag = 0, dynamicProps = null) {
    ...
    if (props !== null) {
        ...
        if (klass != null && !shared.isString(klass)) {
            props.class = shared.normalizeClass(klass);
        }
        if (shared.isObject(style)) {
            // reactive state objects need to be cloned since they are likely to be
            // mutated
            if (reactivity.isReactive(style) && !shared.isArray(style)) {
...
function resolveProps(instance, rawProps, _options) {
    ...
    if (rawProps != null) {
        for (const key in rawProps) {
            ...
            if (hasDeclaredProps) {
                ...
                if (shared.hasOwn(options, camelKey)) {
    ...
    if (hasDeclaredProps) {
        ...
        for (let i = 0; i < needCastKeys.length; i++) {
            ...
            if (opt == null)
                continue;
            const isAbsent = !shared.hasOwn(props, key);
            const hasDefault = shared.hasOwn(opt, 'default');
            const currentValue = props[key];
            // default values
            if (hasDefault && currentValue === undefined) {
                const defaultValue = opt.default;
                setProp(key, shared.isFunction(defaultValue) ? defaultValue() : defaultValue);
            }
            // boolean casting
            if (opt[0 /* shouldCast */]) {
                if (isAbsent && !hasDefault) {
                    ...
                    (currentValue === '' || currentValue === shared.hyphenate(key))) {
                    setProp(key, true);
                }
            }
        }
        ...
        if ( rawProps) {
            for (const key in options) {
                ...
                if (opt == null)
                    continue;
                let rawValue;
                if (!(key in rawProps) && shared.hyphenate(key) in rawProps) {
                    rawValue = rawProps[shared.hyphenate(key)];
                }
                else {
                    rawValue = rawProps[key];
                }
                validateProp(key, reactivity.toRaw(rawValue), opt, !shared.hasOwn(props, key));
            }
        }
    }
    else {
    ...
    instance.attrs = attrs || shared.EMPTY_OBJ;
    instance.vnodeHooks = vnodeHooks || shared.EMPTY_OBJ;
}
...
function normalizePropsOptions(raw) {
    ...
    if (shared.isArray(raw)) {
        for (let i = 0; i < raw.length; i++) {
            ...
            if (normalizedKey[0] !== '$') {
                options[normalizedKey] = shared.EMPTY_OBJ;
            }
            else {
...
function getTypeIndex(type, expectedTypes) {
    ...
    else if (shared.isObject(expectedTypes)) {
...
function resolveSlots(instance, children) {
    ...
    instance.slots = slots || shared.EMPTY_OBJ;
}
...
const directiveToVnodeHooksMap = /*#__PURE__*/ [
    ...
    const vnodeHook = (vnode, prevVnode) => {
        ...
        const prevBindings = prevVnode ? prevVnode.dirs : shared.EMPTY_ARR;
...
function createHydrationFunctions({ mt: mountComponent, p: patch, o: { patchProp, createText } }) {
    ...
    const hydrateNode = (node, vnode, parentComponent = null, optimized = false) => {
    ...
    const hydrateElement = (el, vnode, parentComponent, optimized) => {
        ...
        if (patchFlag !== -1 /* HOISTED */) {
            ...
            if (shapeFlag & 16 /* ARRAY_CHILDREN */ &&
                ...
                let next = hydrateChildren(el.firstChild, vnode, el, parentComponent, optimized);
                let hasWarned = false;
                while (next) {
                    hasMismatch = true;
                    if ( !hasWarned) {
    ...
    const hydrateChildren = (node, vnode, container, parentComponent, optimized) => {
        optimized = optimized || vnode.dynamicChildren !== null;
    ...
    const hydrateFragment = (node, vnode, parentComponent, optimized) => {
        const parent = node.parentNode;
        parent.insertBefore((vnode.el = createText('')), node);
        const next = hydrateChildren(node, vnode, parent, parentComponent, optimized);
        parent.insertBefore((vnode.anchor = createText('')), next);
        return next;
    };
    const hydratePortal = (vnode, parentComponent, optimized) => {
        const targetSelector = vnode.props && vnode.props.target;
        const target = (vnode.target = shared.isString(targetSelector)
            ? document.querySelector(targetSelector)
            : targetSelector);
        if (target != null && vnode.shapeFlag & 16 /* ARRAY_CHILDREN */) {
            hydrateChildren(target.firstChild, vnode, target, parentComponent, optimized);
        }
        else {
            warn(`Attempting to hydrate portal but target ${targetSelector} does not ` +
                `exist in server-rendered markup.`);
        }
    };
    const handleMismtach = (node, vnode, parentComponent) => {
        hasMismatch = true;
...
function baseCreateRenderer(options, createHydrationFns) {
    ...
    const setRef = (ref, oldRef, parent, value) => {
        ...
        const refs = parent.refs === shared.EMPTY_OBJ ? (parent.refs = {}) : parent.refs;
...
function instanceWatch(source, cb, options) {
    ...
    const getter = shared.isString(source) ? () => ctx[source] : source.bind(ctx);
...
const PublicInstanceProxyHandlers = {
    get(target, key) {
        ...
        if (key[0] !== '$') {
            ...
            else if (renderContext !== shared.EMPTY_OBJ && shared.hasOwn(renderContext, key)) {
        ...
        else if (shared.hasOwn(sink, key)) {
            return sink[key];
        }
    ...
    has(target, key) {
        ...
        return (accessCache[key] !== undefined ||
            ...
            shared.hasOwn(renderContext, key) ||
            (type.props != null && shared.hasOwn(type.props, key)) ||
            shared.hasOwn(publicPropertiesMap, key) ||
            shared.hasOwn(sink, key));
    },
    set(target, key, value) {
        const { data, renderContext } = target;
        if (data !== shared.EMPTY_OBJ && shared.hasOwn(data, key)) {
            data[key] = value;
        }
        else if (shared.hasOwn(renderContext, key)) {
...
function applyOptions(instance, options, asMixin = false) {
    ...
    const renderContext = instance.renderContext === shared.EMPTY_OBJ &&
    ...
    // state options
    if (dataOptions) {
        const data = shared.isFunction(dataOptions) ? dataOptions.call(ctx) : dataOptions;
...
function createComponentInstance(vnode, parent) {
    ...
    const instance = {
        ...
        renderContext: shared.EMPTY_OBJ,
        data: shared.EMPTY_OBJ,
        props: shared.EMPTY_OBJ,
        attrs: shared.EMPTY_OBJ,
        vnodeHooks: shared.EMPTY_OBJ,
        ...
        emit: (event, ...args) => {
            ...
            let handler = props[`on${event}`] || props[`on${shared.capitalize(event)}`];
            if (!handler && event.indexOf('update:') === 0) {
                event = shared.hyphenate(event);
                handler = props[`on${event}`] || props[`on${shared.capitalize(event)}`];
            }
            if (handler) {
                const res = callWithAsyncErrorHandling(handler, instance, 6 /* COMPONENT_EVENT_HANDLER */, args);
                return shared.isArray(res) ? res : [res];
            }
            else {
                return [];
            }
...
function setupStatefulComponent(instance, parentSuspense, isSSR) {
...
function handleSetupResult(instance, setupResult, parentSuspense, isSSR) {
...
instance) {
    ...
    else if (shared.isFunction(component) || shared.isObject(component)) {
        return component;
    }
}
...
exports.markNonReactive = reactivity.markNonReactive;
exports.markReadonly = reactivity.markReadonly;
