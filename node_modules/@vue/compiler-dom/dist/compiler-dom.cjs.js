...
const parserOptionsStandard = {
    ...
    maxCRNameLength: /*#__PURE__*/ Object.keys(namedCharacterReferences).reduce((max, name) => Math.max(max, name.length), 0)
};
...
function parseInlineCSS(cssText, loc) {
    ...
    return compilerCore.createSimpleExpression(JSON.stringify(res), false, loc, true);
}
...
const stringifyStatic = (node, context) => {
    if (shouldOptimize(node)) {
        return compilerCore.createCallExpression(context.helper(compilerCore.CREATE_STATIC), [
...
function shouldOptimize(node) {
    let bindingThreshold = 5 /* ELEMENT_WITH_BINDING_COUNT */;
    let nodeThreshold = 20 /* NODE_COUNT */;
...
const parserOptions =  parserOptionsStandard;
...
function parse(template, options = {}) {
    return compilerCore.baseParse(template, {
        ...parserOptions,
        ...options
    });
}
